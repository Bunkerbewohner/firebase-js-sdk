...
export class View {
  ...
  /** The last snapshot version that this view has been synced with. */
   ...
   * @param current If this change was triggered by a RemoteEvent, the new
   *        CURRENT state at the time of these changes.
  ...
  computeDocChanges(
    ...
    current = current === undefined ? this.current : current;
      ...
      (key: DocumentKey, newMaybeDoc: MaybeDocument) => {
        ...
        if (oldDoc && newDoc) {
          ...
          if (docsChanged) {
            ...
            const shouldWaitForRemoteVersion =
              newDoc.hasPendingWrites(current) &&
            ...
            oldDoc.hasPendingWrites(current) !==
            newDoc.hasPendingWrites(current)
          ) {
        ...
        if (changeApplied) {
          if (newDoc) {
            ...
            if (newDoc.hasPendingWrites(this.current)) {
  ...
  private shouldBeInLimbo(key: DocumentKey): boolean {
    ...
    if (this.documentSet.get(key)!.hasPendingWrites(this.current)) {
      return false;
    }
